...
// File modified for in-app debug messages only. No functionality changes.
    ...
    public class ImportExportAnchorManager : Singleton<ImportExportAnchorManager>
    {
        ...
        private void Connected(object sender, EventArgs e)
        {
            if (SharingStage.Instance.ShowDetailedLogs)
            {
                ...
                {
                    AnchorDebugText.text += string.Format("\n(1/3): Connecting to Address: " + SharingStage.Instance.ServerAddress);
                }
            }
        ...
        private void RoomManagerCallbacks_AnchorUploaded(bool successful, XString failureReason)
        {
            if (successful)
            {
                ...
                if (SharingStage.Instance.ShowDetailedLogs && AnchorDebugText != null)
                {
                    AnchorDebugText.text += string.Format("\n(3/3): Anchor Upload successful. Now others can join.");
                }
            }
            else
            {
        ...
        private void InitRoomApi()
        {
            ...
            if (currentRoom != null)
            {
                ...
                if (SharingStage.Instance.ShowDetailedLogs)
                {
                    ...
                    if (AnchorDebugText != null)
                    {
                        AnchorDebugText.text += string.Format("\n(2/3): In room: {0} with ID {1} an state {2}",
                            roomManager.GetCurrentRoom().GetName().GetString(),
                            roomManager.GetCurrentRoom().GetID().ToString(),
                            currentState);
                    }
        ...
        private void StartAnchorProcess()
        {
            ...
            if (SharingStage.Instance.ShowDetailedLogs)
            {
                ...
                if (AnchorDebugText != null)
                {
                    AnchorDebugText.text += string.Format("\n(2.5/3): Found " + anchorCount.ToString() + " anchor(s)");
                }
            }
